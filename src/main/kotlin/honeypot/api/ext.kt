...
fun alert(message: Any, info: Any?) =
    window.alert("Error: $message ${if (info == null) "" else JSON.stringify(info)}")
...
fun <T> evaluate(expression: String, onSuccess: (T) -> Unit = {}, onError: (Throwable) -> Unit = {}) {
    if (isChrome)
        chrome.devtools.inspectedWindow.eval<T>(expression, mapOf()) { result, exception ->
            if (exception == null) onSuccess(result) else onError(Exception(exception.description))
        }
    else
